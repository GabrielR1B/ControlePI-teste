...
<div class="actions">
	<ul>
		...
		<li><?php echo $this->Html->link(__('Deletar DI', true), array('action' => 'delete', $this->Form->value('Desenho.id')), null, __('Você tem certeza de que deseja excluir o desenho industrial em questão?', true)); ?></li>
	</ul>
</div>
<div class="desenhos form">
<?php echo $this->Form->create('Desenho', array('enctype' => 'multipart/form-data'));?>
	<fieldset>
		<legend><?php __('Editar Desenho Industrial'); ?></legend>
	<?php
	...
	<div id="lista_departamentos">
		...
		<?php if (!empty($departamentos)): ?>
			<?php $count = count($departamentos) ?>
				...
				<?php endforeach; ?>
		<?php else: ?>
			<li>Não há departamentos associados a este desenho industrial</li>
		<?php endif;?>
		</ul>
	</div>
	...
	<div id="lista_titulares">
		...
		<?php if ( !empty($titulares)){ ?>
			<?php $count = count($titulares) ?>
				...
				<?php endforeach; ?>
		<?php }else{ ?>
			<li>Não há titulares associados a este desenho industrial</li>
		<?php } ?>
		</ul>
	</div>
	...
	<div id="lista_inventores">
		...
		<?php if (!empty($inventores)): ?>
			<?php $count = count($inventores) ?>
				...
				<?php endforeach; ?>
		<?php else: ?>
			<li>Não há inventores associados a este desenho industrial</li>
		<?php endif; ?>
		</ul>
	</div>
...
<fieldset>
		...
		<div id="lista_arquivos">
			...
			<?php }else{ ?>
				<li>Não há arquivos anexados a este desenho industrial</li>
			<?php } ?>
			</ul>
			
			<form class="editFileForm" id="editAddFileForm" tecnologia_id="<?php echo $this->data['Desenho']['id']; ?>" action="" method	="post" enctype="multipart/form-data">
					<input type="file" name="data[Desenho][arquivo]">
					<div class="submit">
...
//////////////////////////	
	...
	function associarInventor( nome, id ) {
		console.log(nome,id);
		jQuery.ajax({
			url: "<?php echo $this->Html->url( array('controller'=>'desenhos', 'action'=>'ajaxAssociarInventor') ) ?>",
			data: "id=" + id + "&tec_id=<?php echo $desenho_id ?>",
			dataType: "json",
			invokedata: {
	...
	$("#buscar_inventor").autocomplete({
		source: "<?php echo $this->Html->url( array('controller'=>'desenhos', 'action'=>'ajaxListarInventores') ) ?>",
		minLength: 2,
...
//////////////////////////
	...
	jQuery("#lista_inventores li a").live( 'click', function(event) {
		...
		confirma = confirm("<?php echo __('Tem certeza que deseja desassociar este inventor deste desenho industrial?', true) ?>");
		if (!confirma) {
		...
		jQuery.ajax({
			url: "<?php echo $this->Html->url( array('controller'=>'desenhos', 'action'=>'ajaxDesassociarInventor') ) ?>",
			data: "tec_id=<?php echo $desenho_id ?>&id=" + $(this).attr('name'), // pega o ID q está armazenado no name attribute
			dataType: "json",
			invokedata: {
	...
	function exibirContratoCotitularidade(){
		if($( ".titular" ).length > 0){
			$('#DesenhoJustificativaCotitularidade').parent().show();
			$('#DesenhoContratoCotitularidade').parent().show();		
		}else{
			$('#DesenhoJustificativaCotitularidade').parent().hide();
			$('#DesenhoContratoCotitularidade').parent().hide();
		}
	}
...
//////////////////////////	
	...
	function associarEmpresa(nome, id, tipo_vinculo){
		jQuery.ajax({
			...
			data: "empresa_id=" + id + "&natureza_id=3" +"&pi_id=<?php echo $desenho_id  ?>" + "&tipo_relacao_id=" + tipo_vinculo,
			dataType: "json",
			type: 'GET',
			//invokedata: {
			//	data1: "<li>" + nome + "<a href='javascript:void(0);' name='" + id + "'></a></li>"
...
//////////////////////////	
	...
	function associarDepartamento( nome, id, unidade_id ) {
		jQuery.ajax({
			url: "<?php echo $this->Html->url( array('controller'=>'desenhos', 'action'=>'ajaxAssociarDepartamento') ) ?>",
			data: "id=" + id + "&tec_id=<?php echo $desenho_id ?>&uni_id=" + unidade_id,
			dataType: "json",
			invokedata: {
				data1: "<li>" + nome + "<a href='javascript:void(0);' name='" + id + "'></a></li>"
			},
	...
	$("#buscar_departamento").autocomplete({
		source: "<?php echo $this->Html->url( array('controller'=>'desenhos', 'action'=>'ajaxListarDepartamentos') ) ?>",
		minLength: 2,
		select: function( event, ui ) {
...
/////////////////////////////
	...
	jQuery("#lista_departamentos li a").live( 'click', function(event) {
		...
		confirma = confirm("<?php echo __('Tem certeza que deseja desassociar este departamento deste desenho industrial?', true) ?>");
		if (!confirma) {
			// se o usuário desistir de excluir, retorna e interrompe a funcao.
			return;
		}
		...
		jQuery.ajax({
			url: "<?php echo $this->Html->url( array('controller'=>'desenhos', 'action'=>'ajaxDesassociarDepartamento') ) ?>",
			data: "tec_id=<?php echo $desenho_id ?>&id=" + $(this).attr('name'), // pega o ID q está armazenado no name attribute
			dataType: "json",
			invokedata: {
				data1: $(this) // envia este objeto q armazena o item clicado, assim posso apagar o elemento após o retorno do ajax call
...
//////////////////////////	
	...
	function associarTitular( nome, id ) {
		jQuery.ajax({
			url: "<?php echo $this->Html->url( array('controller'=>'desenhos', 'action'=>'ajaxAssociarTitular') ) ?>",
			data: "id=" + id + "&tec_id=<?php echo $desenho_id ?>",
			dataType: "json",
			invokedata: {
	...
	$("#buscar_titular").autocomplete({
		source: "<?php echo $this->Html->url( array('controller'=>'desenhos', 'action'=>'ajaxListarTitulares') ) ?>",
		minLength: 2,
		select: function( event, ui ) {
...
//////////////////////////
	...
	jQuery("#lista_titulares li a").live( 'click', function(event) {
		...
		confirma = confirm("<?php echo __('Tem certeza que deseja desassociar este titular deste desenho industrial?', true) ?>");
		if (!confirma) {
			// se o usuário desistir de excluir, retorna e interrompe a funcao.
			return;
		}
		...
		jQuery.ajax({
			url: "<?php echo $this->Html->url( array('controller'=>'desenhos', 'action'=>'ajaxDesassociarTitular') ) ?>",
			data: "tec_id=<?php echo $desenho_id ?>&id=" + $(this).attr('name'), // pega o ID q está armazenado no name attribute
			dataType: "json",
			invokedata: {
				data1: $(this) // envia este objeto q armazena o item clicado, assim posso apagar o elemento após o retorno do ajax call
...
$('#editAddFileForm').ajaxForm({
	...
	url: base_url + "/desenhos/ajaxAdicionarArquivos/" + $('#editAddFileForm').attr('tecnologia_id'),
	dataType: "json",
