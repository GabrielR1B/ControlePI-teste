...
class MarcasController extends AppController {
	...
	function search() {		
		...
		if (!empty($this->data)) {
			...
			$titular_id = $this->data['Marca']['titular_id'];
			$status_id = $this->data['Marca']['status_id'];
			$status_transferencia_id = $this->data['Marca']['status_transferencia_id'];
			$andamento_id = $this->data['Marca']['andamento_id'];
			
			if (!empty($this->data['Marca']['desde'])) {
				$dataDesde = date("Y-m-d", strtotime($this->data['Marca']['desde']));
			}

			if (!empty($this->data['Marca']['ate'])) {
				$dataAte = date("Y-m-d", strtotime($this->data['Marca']['ate']));
			}
			...
			if (!empty($titular_id)) {
				array_push($options['joins'],
					array('table' => 'marcas_titulares',
						'alias' => 'MarcasTitulares',
						'type' => 'inner',
						'conditions' => array(
							'Marca.id = MarcasTitulares.marca_id'
						)
					)
				);
				$condicoes['MarcasTitulares.titular_id'] = $titular_id;
			}

			if (!empty($status_id)) {
				$condicoes['Marca.status_id'] = $status_id;
			}

			if (!empty($status_transferencia_id)) {
				$condicoes['Marca.'.$status_transferencia_id] = 1;
			}

			if (!empty($andamento_id)) {
				$condicoes['Marca.andamento_id'] = $andamento_id;
			}
		
			if (!empty($dataDesde) && !empty($dataAte)) {
				$condicoes['Marca.data >='] = $dataDesde;
				$condicoes['Marca.data <='] = $dataAte;
			} elseif (!empty($dataDesde) && empty($dataAte)) {
				$condicoes['Marca.data >='] = $dataDesde;
			}	elseif (empty($dataDesde) && !empty($dataAte)) {
					$condicoes['Marca.data <='] = $dataAte;
			}
		...
		$this->loadModel('Andamento');

		$titulares = $this->Marca->Titular->find('list');
		$status = $this->Marca->Status->find('list');
		$unidades = $this->Unidade->find('list');
		$departamentos = $this->Departamento->find('list');
		$andamentos = $this->Andamento->find('list');
		...
		$this->set(compact('desdes', 'ates', 'colunas','titulares','status','unidades','departamentos','andamentos'));
	}
