...
class InventoresController extends AppController {
	...
	function add() {
		if (!empty($this->data)) {	
			$this->data['Inventor'] = array_filter($this->data['Inventor'], function($value) { return !is_null($value) && $value !== ''; });

			//Valida o campo Nome		
			if(is_null($this->data['Inventor']['nome']) || empty($this->data['Inventor']['nome']) || !isset($this->data['Inventor']['nome']))
			{
				$this->Session->setFlash('O campo Nome Completo é obrigatório.');	
				$this->redirect(array('action' => 'add'));exit;
			}else{
				if ($this->Inventor->findByNome($this->data['Inventor']['nome'])) {
					$this->Session->setFlash('Já há no sistema um inventor com este nome');	
					$this->redirect(array('action' => 'add'));exit;
				}
			}

			//Valida o campo CPF
			if(isset($this->data['Inventor']['cpf']))
			{
				$this->data['Inventor']['cpf'] = preg_replace('/[^0-9]+/', '', $this->data['Inventor']['cpf']);
				if ($this->Inventor->findByCpf($this->data['Inventor']['cpf'])) {
					$this->Session->setFlash('Já há no sistema um inventor com este CPF');	
					$this->redirect(array('action' => 'add'));exit;
				}
			}
