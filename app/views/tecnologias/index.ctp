...
<div class="tecnologias index">
	<h2><?php __('Patentes Nacionais');?></h2>
	<p>
		<?php
	...
	<tr>
		...
		<th><?php echo $this->Paginator->sort('andamento_id');?></th>
		<?php
			if($session->read('Auth.User.group_id')==1){
	...
	<tr<?php echo $class;?>>
		...
		<td><?php echo $this->Html->link($tecnologia['Andamento']['nome'], array('controller' => 'areas', 'action' => 'view', $tecnologia['Andamento']['id'])); ?></td>			
			<?php			
				if($session->read('Auth.User.group_id')==1){
