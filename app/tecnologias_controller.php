...
class TecnologiasController extends AppController {
	...
	function search() {		
		...
		if (!empty($this->data)) {
			...
			$user_id = $this->data['Tecnologia']['user_id'];
			...
			if (!empty($status_transferencia_id)) {
				$condicoes['Tecnologia.status_transferencia_id'] = $status_transferencia_id;
			}
			...
			if (!empty($user_id)) {
				$condicoes['Tecnologia.user_id'] = $user_id;
			}
		...
		$this->set(compact('users','areas','areas_conhecimento','titulares','status_transferencia','unidades','departamentos', 'status', 'inventores', 'desdes', 'ates','andamentos','populate_palavras_chave','mesDes','mesAtes','colunas'));
	}
